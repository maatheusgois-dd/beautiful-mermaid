      ...
      <div class="config-section">
        ...
        <div class="padding-field">
          <div class="padding-row">
            ...
            <input class="padding-num" id="cfg-edge-stroke" type="number" min="0.25" max="6" step="0.25" value="1" />
          </div>
          <input class="padding-slider" id="cfg-edge-stroke-slider" type="range" min="0.25" max="6" step="0.25" value="1" />
        </div>
        <div class="padding-field">
          <div class="padding-row">
            <label>Node border</label>
            <input class="padding-num" id="cfg-node-stroke" type="number" min="0.25" max="6" step="0.25" value="1" />
          </div>
          <input class="padding-slider" id="cfg-node-stroke-slider" type="range" min="0.25" max="6" step="0.25" value="1" />
        </div>
      </div>
...
// Apply stroke-width overrides directly on SVG elements, skipping <defs>
// (markers, arrow heads, clip paths live in defs and must not be touched).
function applyStrokeOverrides(svgEl) {
  ...
  if (cfgEdgeStroke !== 1) {
    ...
    // Connectors are lines, fill=none paths/polylines
    ...
    // markerUnits="strokeWidth" (default): both the viewport AND the polygon
    // coordinate space scale with stroke-width, so reducing markerWidth just clips.
    // Fix: add a viewBox to decouple polygon coords from the viewport, then switch to
    // userSpaceOnUse with absolute sizes = origSize * sqrt(ew).
    // sqrt damping: at stroke=4 arrows are 2x original, at stroke=6 ~2.45x.
    ...
    svgEl.querySelectorAll('defs marker').forEach(function(marker) {
      ...
      // viewBox maps the polygon's local coordinate space (0..origW × 0..origH)
      // into whatever markerWidth/markerHeight we set — refX/refY stay unchanged
      // because they reference the viewBox coordinate system.
  ...
  if (cfgNodeStroke !== 1) {
    ...
    // Node borders: rect, ellipse, circle, filled polygons — all outside defs
